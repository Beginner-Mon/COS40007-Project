...
print("[phase=plot] Red dashed lines indicate activity_type transition points.")
...
print("[phase=loss] Using FocalLoss (alpha=0.25, gamma=2.0) for multiclass activity_type classification")
...
print("[phase=dataset] Initialize containers for 10-fold CV")
cv_fold_metrics = []
oof_true = []
oof_pred = []
...
for fold_idx, (train_idx, val_idx) in enumerate(fold_splits, start=1):
    ...
    checkpoint_path = PROJECT_ROOT / "notebook" / f"best_bilstm_activity_type_fold_{fold_idx}.pt"
...
if len(oof_true) > 0:
    ...
    print("\n[phase=eval] CV fold metrics (mean ± std):")
    print(f"val_loss: {cv_results_df['best_val_loss'].mean():.4f} ± {cv_results_df['best_val_loss'].std():.4f}")
    print(f"val_acc : {cv_results_df['best_val_acc'].mean():.4f} ± {cv_results_df['best_val_acc'].std():.4f}")
else:
